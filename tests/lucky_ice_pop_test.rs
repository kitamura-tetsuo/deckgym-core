...
mod tests {
    ...
    let actions = generate_possible_trainer_actions(
        &state,
        trainer_card
    );
    ...
    let action = Action {
        actor: 0,
        action: SimpleAction::Play { trainer_card: trainer_card.clone() },
        is_stack: false,
    };
    ...
    assert!(found_heads, "Should check that heads is possible (return to hand)");
