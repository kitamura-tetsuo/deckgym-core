...
use deckgym::encoding::{action_name, encode_action, encode_observation, get_offset_apply_damage};
use deckgym::test_helpers::load_test_decks;
use deckgym::{card_ids::CardId, State};
...
fn test_known_cards_encoding() {
    ...
    let visible_deck_id =
        CardId::from_card_id(&visible_deck_card.get_id()).unwrap() as usize as f32;
    ...
    let visible_hand_id =
        CardId::from_card_id(&visible_hand_card.get_id()).unwrap() as usize as f32;
    ...
    assert!(
        obs.contains(&visible_deck_id),
        "Observation should contain visible deck card ID"
    );
    assert!(
        obs.contains(&visible_hand_id),
        "Observation should contain visible hand card ID"
    );
