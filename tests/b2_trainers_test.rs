...
fn test_piers_requires_obstagoon_and_discards_energy() {
    ...
    let play_action = actions
        .iter()
        .find(|a| matches!(a.action, SimpleAction::Play { .. }))
        .expect("Piers should be playable");
...
fn test_sightseer_draws_stage1_pokemon() {
    ...
    let play_action = actions
        .iter()
        .find(|a| matches!(a.action, SimpleAction::Play { .. }))
        .expect("Sightseer should be playable");
...
fn test_juggler_requires_3_types_and_moves_energy() {
    ...
    state.in_play_pokemon[0][0]
        ...
        .unwrap()
        .attached_energy
        .push(EnergyType::Fire);
    state.in_play_pokemon[0][0]
        .as_mut()
        .unwrap()
        .attached_energy
        .push(EnergyType::Psychic);

    let (_, actions) = generate_possible_actions(&state);
    let play_action = actions
        .iter()
        .find(|a| matches!(a.action, SimpleAction::Play { .. }))
        .expect("Juggler should be playable now");
