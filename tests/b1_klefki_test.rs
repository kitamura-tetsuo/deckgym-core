...
fn test_klefki_dismantling_keys_discards_tool_and_self() {
    ...
    assert!(
        ...
        "Klefki's ability should be available"
    );
    ...
    assert!(
        ...
        "Tool should be discarded from opponent's active Pokemon"
    );
    assert_eq!(
        state.discard_piles[1].len(),
        1,
        "Discard pile for Player 1 should receive the tool"
    );
    if let deckgym::models::Card::Trainer(tc) = &state.discard_piles[1][0] {
        assert_eq!(
            tc.name, "Rocky Helmet",
            "Discarded tool should be Rocky Helmet"
        );
    } else {
    ...
    assert!(
        ...
        "Bench slot 1 should be empty, Klefki discarded"
    );
    assert_eq!(
        state.discard_piles[0].len(),
        1,
        "Klefki should be in Player 0's discard pile"
    );
}
...
fn test_klefki_dismantling_keys_cannot_be_used_if_no_tool() {
    ...
    assert!(
        !possible_moves
            ...
            .iter()
            .any(|a| matches!(a.action, SimpleAction::UseAbility { in_play_idx: 1 })),
        "Klefki's ability should NOT be available if opponent has no tool"
    );
}
