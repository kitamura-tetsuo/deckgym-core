...
mod tests {
    ...
    fn test_cloyster_reduced_damage() {
        // Arrange
        let mut state = State::default();
        let attacker = get_card_by_enum(CardId::A1001Bulbasaur); // attack is 40
        let played_attacker = to_playable_card(&attacker, false);
        state.in_play_pokemon[0][0] = Some(played_attacker);

        let defender = get_card_by_enum(CardId::A1067Cloyster); // -10 damage
        let played_defender = to_playable_card(&defender, false);
        state.in_play_pokemon[1][0] = Some(played_defender);

        // Act
        let damage_with_shell_armor = modify_damage(&state, (0, 0), (40, 1, 0), true, None);

        // Assert
        assert_eq!(
            damage_with_shell_armor, 30,
